...
// TODO: put these into a class and make static so I don't need to forward declare these
shared_ptr<ASTNode> parseExpression(Parser& p, TokenIter& iter, const TokenIter& end);
shared_ptr<ASTNode> parseExpressionPrimary(Parser& p, TokenIter& iter, const TokenIter& end);
shared_ptr<ASTNode> parseOperator(Parser& p, TokenIter& iter, const TokenIter& end, shared_ptr<ASTNode> lhs, int precedence);

shared_ptr<BlockNode> parseBlock(Parser& p, TokenIter& iter);
shared_ptr<StatementNode> parseStatement(Parser& p, TokenIter& iter);
shared_ptr<AssignmentNode> parseAssignment(Parser& p, TokenIter& iter);
shared_ptr<DeclarationNode> parseDeclaration(Parser& p, TokenIter& iter);
shared_ptr<ControlFlowNode> parseControlFlow(Parser& p, TokenIter& iter);
...
shared_ptr<ASTNode> parseOperator(Parser& p, TokenIter& iter, const TokenIter& end, shared_ptr<ASTNode> lhs, int min_precedence) {
	while (true) {
		auto token = *iter;
...
pair<shared_ptr<ASTNode>, int> Parser::parse(TokenIter begin, TokenIter end) {
	...
	auto root = parseExpression(*this, begin, end);
